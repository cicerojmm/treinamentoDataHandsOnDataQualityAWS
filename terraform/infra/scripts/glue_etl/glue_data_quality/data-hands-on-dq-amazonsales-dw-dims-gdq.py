...
def ler_dimensoes(spark, path_base):
    return {
        "dim_user": spark.read.format("delta").load(f"{path_base}/dim_user"),
        "dim_product": spark.read.format("delta").load(f"{path_base}/dim_product"),
        "dim_rating": spark.read.format("delta").load(f"{path_base}/dim_rating"),
    }
